...
{
    uint32_t max_score   = 0;       // Find highest bucket count for scaling later
    uint32_t total_score = 0;       // Total of all counts

    for ( uint32_t bucket = 0; bucket <= max_bucket_; bucket++ )
    {
    ...
    snprintf( buffer, sizeof( buffer), "%-11.11s[Count][%%      ][Cum%%   ]\n", title_.c_str()  );
    report += buffer;
    
    double cumulative_fraction = 0.0;
    
    for ( uint32_t bucket = 0; bucket <= max_bucket_ + 1; bucket++ )
    {
        uint32_t score = buckets_[ bucket ];
...
{
    char buffer[ 50 ];
    snprintf( buffer, sizeof( buffer ), "%3.2f", percentage );
